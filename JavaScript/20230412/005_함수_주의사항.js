...
foo()     // error 밖으로 못빠져나와서(이 스코프 자체가 함수이기때문에)
          // 즉시 실행함수로 실행시켰을 경우 밖에서 호출할 수 없습니다. 

// 이런 함수가 어디서 쓰일까요?
// 1. 메모리 효율적으로 관리하기 위해  2. 바로 실행해야 하는 것을 즉시 실행함수로 관리
...
// call by value(다른언어에서 값의 전달을 표현하기위해 사용)
// call by reference(참조-다른언어에서 주소값의 전달을 표현하기위해 사용)
...
test(a)
a         // {}  // 다른언어의 겨우 call by ref로 동작하여 a의 값이 1000으로 바귑니다. 
