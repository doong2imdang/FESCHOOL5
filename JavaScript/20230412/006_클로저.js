...
// 함수를 리턴해줄 수 있음을 나타내는 예제 (클로저와 상관 x)
function 리턴함수(){
  function sum(a, b) {
    return a + b
  }
...
var newInner = outer();   // 휘발 x - newInner가 있기때문에 언제든지 호출될 수도 있으니.
newInner();             // 12
...
function 제곱(x){
  ...
  return 승수   // 함수이름 자체인것 호출하려면 승수()이렇게 생겨야 함
}
