...
// 함수의 일반적인 형태
// 함수 선언문
// 파선아실  a,b는 파라미터   // 7, 3 을 argument 라고 한다.  one은 함수
function one(a, b){   // 1 
  let z = a + b       // 2
  return z ** 2       // 3
}
...
// 화살표 함수   input     output(return문이 바로 있어서 괄호 생략략)
const two = (a, b) => (a + b) ** 2
console.log(two(7, 3))
console.log(two(7, 3))
...
// 아래와 같이 햇을 경우에는 함수의 순수성, 순수함수의 장점을 살릴 수 없습니다. (c가 있는데 c를 안쓰고!!)
// 외부에서 직접 값을 가져오는 것을 지양해주세요. 파라미터 값을 넣어주세여~
function four(a, b, c){
...
let a = hello1()    // undefined
let b = hello3()    // undefined
let c = hello3()   // undefined


// return 문을 만나면 중간에 종료를 하게 된다.
function hello4(){
  console.log('hello')
  console.log('hello')
  console.log('hello')
...
함수1(10, 20, 30, 40)  // error가 발생하지 않습니다. // 60 다른언어에서는 에러

함수1(10, 20)    // error가 발생하지 않습니다. // NaN
...
//a와 c에 들어갈 것이라고 생각햇지만 a와 b에 들어갑니다. 
함수1(a=1, c=1)    // 32


// 아래와 같은 식별이슈가 있을 경우 object로 넘깁니다. roro기법
...
// 기본값 설정(roro기법)
function runPython({
  user = '',     // 구문 자체가 선언이라서 user : 0이 아닌것이다.
  time = 0, 
  code = '', 
  lv = 0 }) {
}
...
let 함수2 = x => {
  if (true) {
    console.log('hello world')   // if 문 이런경우에는 중괄호 필수
  }
  return x + 10
}
...
함수['location'] = 'jeju'  // 'jeju'  -> dir를 보면 location이 추가
console.dir(함수)     // location : "jeju"
typeof(함수)    // function

함수['name'] = 'jejufunction'
console.dir(함수)   // name :"함수"  -> name은 변경이 안되는 것을 볼 수 있다.
...
// 3. roro기법 원리 설명
// roro 기법은 무엇인가요? 입력되는 아규먼트가 매우 많은 함수의 경우 호출하는 쪽에서 어떤 값이 들어가는지 명확히 알기 어렵고 읽기도 어렵다. (검색: 자바스크립트 로로 패턴)
window.addNewControl("Title", 20, 50, 100, 50, 100, 200, 300, true);
// 여러분들이 코드를 쭉 읽어 내려가다가 hello함수를 만났습니다. 기분이 어떨 것 같으신가요? (물론 저렇게 함수명을 짜면 힘듭니다.)
...
// Silver true true true 3일 이내 가능 false  // 순서가 바뀌고 중간에 생략되어도 잘 들어간다.

// 원리(모르셔도 코딩하는데 지장이 있진 않습니다.)
let one
let two
let three
...
let {one, two, three} = {two: 20, one: 10, three: 30}  // 순서 뒤바껴도 결과 나오는데는 지장x
...
로그인정보()  // error
로그인정보({})  // 출력
// 위 코드에서 로그인정보({}) 호출했을 때는 error가 안나지만 로그인정보()호출했을 경우에는 error가 납니다. 
...
함수()   // 6
함수({})  // 6
함수({b:100})  // 104  // b에 제대로 할당이 되었다. 라는 것을 알 수 있다. 
