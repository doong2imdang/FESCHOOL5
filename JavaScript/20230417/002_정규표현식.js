...
'puallab CEO leehojun hello CEO'
...
'puallab CEO leehojun hello CEO'.match(/CEO/g)   // ['CEO', 'CEO']

// 특정한 패턴으로 Split
'puallab CEO leehojun hello CEO'.split(/CEO/g)   // ['puallab ', ' leehojun hello ', '']

// 패턴이 들어가 있으면 true 없으면 false
(/CEO/g).test('puallab CEO leehojun hello CEO')  // true
...
'!!!'.split('!')  // 4개  // ['', '', '', '']
'!a!a!a'.split('!')  // 4개  // ['', 'a', 'a', 'a']
'a!a!a!'.split('!')  // 4개  // ['a', 'a', 'a', '']

'!a!a!a'.split('!').length - 1  // 원하는 개수가 나온다. -1을 해줘야

// 3. 일반문자열
/ hello / g

// 4. 처음과 끝
/^ hello / g  // hello로 시작하는 거
/ hello$ / g  // hello로 끝나는 거
/^ hello$ / g  // hello로 시작하고 끝나는거

// 5. 모든문자 매핑(.)
/ hello / g
/ h.llo / g  // hello, hallo, hollo...
/ hello..world / g  // hello  world(공백 두칸), hello, world(,공백)...
...
//// 1. https://school.programmers.co.kr/learn/courses/30/lessons/120826 ////
...
//// 2. https://school.programmers.co.kr/learn/courses/30/lessons/120851 ////
// 정답유형1 : 숫자를 찾는다!
...
"aAb1B2cC34oOp".match(/[0-9]/g) // ['1', '2', '3', '4']
"aAb1B2cC34oOp"
    .match(/[0-9]/g)
    .map(x => parseInt(x))
    .reduce((a, b) => a + b, 0)
...
"aAb1B2cC34oOp"	
    .replace(/[^0-9]/g, '')
    .split('')  // reduece는 array를 더해주는 것이기때문에
    .reduce((a, b) => parseInt(a) + parseInt(b), 0)


//// 3. https://school.programmers.co.kr/learn/courses/30/lessons/120891 ////
// 오답 
function solutionI(order) {
...
function solution(order) {
  return String(order)
    .replace(/[369]/g, '').length;
}

//// 4. https://school.programmers.co.kr/learn/courses/30/lessons/120913 ////
...
"abc1Addfggg4556b".match(/[a-zA-Z0-9]{1,4}/g)  // ['abc1', 'Addf', 'ggg4', '556b']
...
// 오답
function solution(my_str, n) {
  let reg = new RegExp(`.{${n}}`, 'g')  // 마지막 남은 것도 리턴해주서야 해서 오답
...
//// 5. https://school.programmers.co.kr/learn/courses/30/lessons/120864 ////
// 2번 문제 업그레이드
// 2번 문제 해답
...
"aAb1B2cC34oO123p".match(/\d+/g);  //  ['1', '2', '34', '123']
...
'87a99b00fww89e'.match(/[0-9][0-9][a-zA-Z]/g)  // ['87a', '99b', '00f', '89e']
...
'a10b9r10ce33uab8wc918v2cv11v9'.match(/[rev]10?[0-9]/g); // ['r10', 'v11']
