...
export default function Main(props) {
  ...
  const [trainingTime, setTrainingTime] = useState(0);  // placeholder를 설정하기 위해 문자열로
  ...
  return (
    <main>
      <h2 className="a11y-hidden">필요한 시간 알아보기기</h2>
      <form className="cont-input" onSubmit={handleSubmit}>
        ...
        <p className="txt-time">그래서 앞으로 매일 하루에
          <input type="number" required value={trainingTime} onChange={(event) => setTrainingTime(event.target.value)}/>시간씩 훈련할 것이다.
        </p>
      ...
      {!!dday && <section className="cont-result">
        ...
        <p className="txt-wannabe">
          ...
          대략 <strong>{dday}</strong>일 이상 훈련하셔야합니다! :&#41;
        </p>
